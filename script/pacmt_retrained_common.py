...
def load_binary_model(backbone_dir, ckpt_dir, device):
    ...
    # train_binary.py saves AutoModelForSequenceClassification weights with a
    # ``bert.`` prefix, whereas this inference wrapper stores its encoder as
    # ``backbone``.  Normalize that checkpoint format before loading.
    if any(str(key).startswith("bert.") for key in state):
        state = {("backbone." + str(key)[5:] if str(key).startswith("bert.") else key): value
