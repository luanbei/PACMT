...
├── .gitignore
├── script/
│   ├── pacmt_retrained_common.py
│   ├── predict_bina_hiera.py
│   ├── predict_binary.py
│   ├── predict_hierarchy.py
│   ├── train_binary.py
│   ├── train_hierarchy.py
│   └── train_cluster_disjoint_earlystop.py
├── examples/
│   ├── example.fasta
│   └── example.csv
└── docs/
...
`pacmt_retrained_common.py` is required by all three prediction scripts. `train_hierarchy.py` is the multi-GPU launcher and requires `train_cluster_disjoint_earlystop.py` in the same `script/` directory.
...
The binary training script reads `train.csv`, `dev.csv` and `test.csv` from the specified data directory. The example below launches four GPU processes; change `--nproc_per_node` to match the available GPUs.
...
```bash
torchrun --nproc_per_node=4 script/train_binary.py \
...
The hierarchical training script trains four classification heads corresponding to order, family, genus and species. It launches multiple GPU processes and imports `script/train_cluster_disjoint_earlystop.py`, which must remain in the same directory.
...
```bash
torchrun --nproc_per_node=4 script/train_hierarchy.py \
  --model_name_or_path /path/to/DNABERT-2 \
